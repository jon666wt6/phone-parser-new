...
async function populateAssets(targetUrl, saveAssetsFlag) {
  let proccess;
  proccess = { instanceId: "populate-assets", test: true, proxies: ['lightning'] };
  let browser = null;

  try {
    const { browser: newBrowser, page } = await initializeBrowserSession('proxy6', 0, 0, '2082', 4);
    browser = newBrowser;
...
const saveAssetsFlag = process.argv[3] === "save_assets";
...
populateAssets(targetUrl, saveAssetsFlag);